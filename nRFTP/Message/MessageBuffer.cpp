...
void MessageBuffer::printMessageBuffer() {

	RFLOGLN("-------- print msgbuffer -------");
	for(int i = 0; i < MessageBuffer::size; i++) {
		RFLOG("SRC: "); 			RFLOGLN(elements[i].srcAddress);
		RFLOG("DEST: "); 			RFLOGLN(elements[i].destAddress);
		RFLOG("MSG ID: "); 		RFLOGLN(elements[i].messageId);
		RFLOG("FT"); 				RFLOGLN(elements[i].flagsAndType);
		RFLOGLN("");
	}
	RFLOGLN("---------------END---------------");
}
